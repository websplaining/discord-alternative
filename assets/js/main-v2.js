...
(function () {
  ...
  // deep links: #mumble / #ts3 / #ts6 open the matching tab
  function tabFromHash() {
    var h = location.hash.replace('#', '');
    if (!panels[h]) return false;
    var wasHidden = panels[h].hidden;
    activateTab(h);
  ...
  // buttons with data-tab-link switch tabs and scroll to guides
  ...
  // ---- video toggles (collapsed walkthroughs) ----
  document.querySelectorAll('.video-toggle').forEach(function (wrap) {
    var head = wrap.querySelector('.video-head');
    var body = wrap.querySelector('.video-body');
    if (!head || !body) return;
